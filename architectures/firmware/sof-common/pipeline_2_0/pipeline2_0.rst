...
Each module works in its own preemptible thread context, with lower priority than LL. Each of DP module’s thread has the same priority. If there are more than one module ready, the scheduler will choose a module that have the closest deadline, where a deadline is the last moment when a following module will have to finish processing and provide data chunk to next module. Modules will be scheduled for processing when 
...
Current limitation/simplification: DP module must be surrounded by LL modules, there’s no possibility to connect DP to DP. 
...
**(1.0 compatibility)**  To keep compatibility with legacy modules that must use comp_buffer, a hybrid buffer need to be used (described later). Hybrid buffering requires additional data copy operation from 2.0-compatible buffers from/to legacy comp_buffer. This operation is performed by LL processing,
...
serious issue - there's no simple way to use other buffering than comp_buffer. 
...
Lockless cross core ring buffer
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

*already implemented as ring_buffer*
...
Linearization cross core buffer (2.0 new)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
